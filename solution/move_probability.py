...
def calculate_probability_basic_heuristic(ant: Ant, possible_moves: [], graph: np.array, pheromone: np.array, config: Config):
    ...
    for move in possible_moves:
        probabilities.append((graph[ant.current_position][move][0] ** config.heuristic_exp) *
                             (pheromone[ant.current_position][move] ** config.pheromone_exp))
        sum += ((graph[ant.current_position][move][0] ** config.heuristic_exp) *
                (pheromone[ant.current_position][move] ** config.pheromone_exp))
...
def calculate_probability_exploitatioin_heuristic(ant: Ant, possible_moves: [], graph: np.array, pheromone: np.array, config: Config):
    probabilities = []
...
def calculate_probability(ant: Ant, possible_moves: [], config: Config):
    if ant.heuristic == "basic":
        ...
        return calculate_probability_exploitatioin_heuristic(ant, possible_moves, ant.graph, ant.pheromone, config)
